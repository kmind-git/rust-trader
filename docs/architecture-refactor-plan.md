...
状态：分阶段实施；行情快照与 FIX 回报路径已落地，其余架构仍为提案。日期：2026-09-15。

建议采用“单进程、模块化单体、确定性核心、独立 FIX 状态机”的架构。先隔离职责，再将引擎状态收敛到专属线程；保留同步网络和当前盘口结构。专属线程用于统一状态所有权和生命周期，不预先承诺提高吞吐，性能关卡未通过时保留较早阶段的调用方式。

2026-09-15 实施进展：行情读取部分已按后续确认的方案先行落地，采用按品种的 ArcSwap 完整快照，并合并单个操作中的重复深度构建。EngineHost、FIX 状态机拆分等其余阶段仍为方案。实现边界见 [行情快照说明](D:/projects/zcodeworkspace/rust-trader/docs/market-data-snapshots.md)。

同日继续完成 FIX 回报直达 writer 的唯一有界出站队列、实际 reader 句柄超时修正及跨握手保留的输入缓冲。保持原线程模型和发送序号所有权；退出时先清理引擎会话，再发送最终 Logout。实施边界与满载交付限制见 [FIX 回报路径说明](fix-delivery.md)。
...
- 现有消息范围内正确使用 FIX 4.2；未支持的消息、字段选项和配置明确拒绝。依据 [FIX 审计范围](fix42-audit.md)，不退回早期 FIX 4.4 行为。
...
| [rest.rs](../src/rest.rs) | 行情通过 ArcSwap 读取完整快照，JSON 在读取后生成；会话列表仍取 Engine 锁 | 行情查询已脱离撮合锁，后续测量序列化、分配与快照发布成本 |
...
额外优先处理的工作量来源：每个连接循环仍调用 DAY 过期扫描，应统一时钟驱动。断连、改单和批量到期中的重复快照构建已合并，保留原 sequence 递增行为。
...
已实施的行情方案使用每品种一个 `ArcSwap<InstrumentSnapshot>`。当前 Engine 的串行修改路径在操作结束时原子发布完整 Book、Stats、版本与发布时间；未来可由 EngineHost 继续持有相同发布入口。`MarketDataReader` 独立读取 Arc，REST 行情查询不取 Engine Mutex、不做盘口深复制。品种目录也以 ArcSwap 发布，只在品种注册时复制目录，行情更新不复制全市场 map。依赖为 `arc-swap`，不再采用原定的 `RwLock<Arc<_>>`。

用户允许跳过行情中间版本。实现保留每次逻辑 sequence 递增和完整成交统计，在单个改单、断连或批量到期操作中仅重建每个受影响品种的最终快照；版本允许跳跃。操作完成即发布，最后一次更新不依赖后续交易或定时器。没有行情历史队列，不消费查询结果；读者持有的旧 Arc 在使用完后回收。暂不增加跨操作定时节流。保证单品种单个快照一致，不承诺多品种或多个 REST 请求的一致切面；REST JSON 字段保持兼容。
...
| R4 专属引擎与读模型 | engine_host、read_model、REST | ArcSwap 行情查询已独立先行；有界命令入口、独立控制路径仍待实施 | 两部分独立验证；EngineHost 正确性和排队/延迟/内存关卡通过后再切换 |
...
本文件是持续更新的架构方案。行情 ArcSwap 发布/查询与操作内快照合并已实施；不代表 EngineHost、FIX 状态机、生命周期等整个重构已完成。各部分验证结果分别记录，不用行情微基准替代整体架构验收。
