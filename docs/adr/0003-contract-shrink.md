...
2026-09-13 的功能评审（grilling 会话）决定：「契约等价」从硬约束降级为**可收缩基线**——逐项评估后，裁剪 SecurityDefinitionRequest (c) 运行期动态建品种（acceptor 处理 + initiator 封装，回包 (d) 因品种下载依赖而保留）、交易所控制台 watch/unwatch（每品种一线程秒级轮询）、client/playback 的 -props 及配套配置文件存在性检查。依据：这三者在工具链中均零调用（已 grep 实证），属纯表面积而非能力；保留项（撤单/改单 F/G、MassQuote ACK (b)、会话维护全套、REST 四端点、-speed）均有互通或功能价值。
